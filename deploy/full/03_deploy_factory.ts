...
const deployFactory: DeployFunction = async function ({
  ...
  if (network.tags["public"]) {
    if (network.name === "polygon") {
      colouredLog.yellow(
        `Deploying Pool factory to: ${factory.address} and waiting for 3 minutes for the contract to be deployed...`
      );
      await delay(180 * 1_000);
    } else if (network.name === "mumbai") {
      colouredLog.yellow(
        `Deploying Pool factory to: ${factory.address} and waiting for 30 seconds for the contract to be deployed...`
      );
