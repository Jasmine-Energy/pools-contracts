...
const deployRetirementService: DeployFunction = async function ({
  ...
  if (network.tags["public"]) {
    if (network.name === "polygon") {
      colouredLog.yellow(
        `Deploying Retirement Service to: ${retirer.address} and waiting for 3 minutes for the contract to be deployed...`
      );
      await delay(180 * 1_000);
    } else if (network.name === "mumbai") {
      colouredLog.yellow(
        `Deploying Retirement Service to: ${retirer.address} and waiting for 30 seconds for the contract to be deployed...`
      );
  ...
  if (network.tags["public"]) {
    ...
    try {
      await run("verify:verify", {
        ...
        constructorArguments: [minter, eat],
      });
