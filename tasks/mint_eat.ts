...
import { encodeEnergyAttributeTokenId } from "@/utils/token_encoding";
...
task("mint", "Mints an EAT")
  ...
  .setAction(
    async (
      ...
      { ethers, deployments, network, getNamedAccounts, }: HardhatRuntimeEnvironment
    ): Promise<void> => {
      if (network.tags["production"]) {
        colouredLog(LogColours.red, "Error: Unable to use mint on production network.");
        return;
      }
      ...
      const id = encodeEnergyAttributeTokenId(
        uuid, taskArgs.registry, new Date(taskArgs.vintage)
      );
      const amount = parseInt(taskArgs.quantity);
      const oracleData =
        "";
      ...
      console.log(`Signature is: ${signature}`);
        ...
        let tx = await minterContract.mint(
          minterAddress,
          id,
          amount,
          [],
