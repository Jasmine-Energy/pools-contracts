...
task("freeze", "Freezes transfers of an EAT")
  ...
  .setAction(
    async (
      ...
      const ownerAddress = await (JasmineEAT__factory.connect(eatSavedAddress, accounts[0])).owner();
      let signer;
      try {
        if (network.tags["local"] && !accounts.find((acc) => acc.address === ownerAddress)) {
        ...
        colouredLog.red(`Error: Could not load signer of EAT owner. Address: ${ownerAddress}`);
        return;
      }
      ...
      colouredLog.blue(
        `Froze tokens of ID ${taskArgs.token} . Tx: ${tx.hash}`
      );
