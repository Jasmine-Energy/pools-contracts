...
abstract contract JasmineFeePool is JasmineBasePool, IFeePool {
    ...
    ) JasmineBasePool(_eat, _poolFactory, _retirementService, _contractName) {} // solhint-disable-line no-empty-blocks
        ...
        returns (uint256[] memory tokenIds, uint256[] memory amounts)
    {
        ...
        if (withdrawalRate() != 0) {
            uint256 feeAmount = JasmineFeePool.withdrawalCost(amount) -
    ...
    function withdrawSpecific(
        ...
        if (withdrawalSpecificRate() != 0) {
            uint256 feeAmount = JasmineFeePool.withdrawalCost(
                tokenIds,
                amounts
     ...
     */
    function _updateWithdrawalRate(
        uint96 newWithdrawalRate,
        bool isSpecific
    ) private {
        if (isSpecific) {
    ...
    function _enforceFeeManagerRole() private view {
        if (!IJasmineFeeManager(poolFactory).hasFeeManagerRole(_msgSender())) {
            revert JasmineErrors.RequiresRole(
                IJasmineFeeManager(poolFactory).FEE_MANAGER_ROLE()
            );
        }
    }
}
