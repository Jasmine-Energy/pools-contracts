 ...
 */
const decodeEnergyAttributeTokenId = (
  ...
  const uuidPacked = ethers.utils
    .hexZeroPad(ethers.utils.hexValue(uuidNumber), 16)
    .slice(2);
  const uuid = [
 ...
 * associated certificateId, vintage, and registry
 */
const encodeEnergyAttributeTokenId = (
...
const encodeOracleData = (
  ...
  const vintageNumber =
    BigInt(Math.ceil(vintage.getTime() / 1_000)) & BigInt(2 ** 40 - 1);
  const fuelTypeNumber =
    BigInt(FuelTypesArray.indexOf(fuelType)) & BigInt(2 ** 32 - 1);
  const certificateTypeNumber =
    BigInt(CertificateArr.indexOf(certificateType)) & BigInt(2 ** 32 - 1);
  const endorsementNumber =
    BigInt(CertificateEndorsementArr.indexOf(endorsement)) &
    BigInt(2 ** 32 - 1);
