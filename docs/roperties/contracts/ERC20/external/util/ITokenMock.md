...
```solidity
function burn(uint256) external nonpayable
```
...
```solidity
function burnFrom(address, uint256) external nonpayable
```
...
```solidity
function initialSupply() external nonpayable returns (uint256)
```
...
```solidity
function isMintableOrBurnable() external nonpayable returns (bool)
```
